...
import {
  nextArea, areaQuestions, nextCue, cueLadder, AREA_LABEL,
...
export function recallPlan({
  history = [], lastUserText = '', analysis = null, memory = null, cannotRecall = false,
} = {}) {
  ...
  if (cannotRecall) {
    const cue = nextCue(analysis, history);
  ...
  if (!state.mayAsk) {
    return {
      state,
      mode: state.questionCount >= QUESTION_LIMIT ? 'wrap_up' : 'invite',
      cue: null, area: null, questions: [],
    };
  }
  ...
  if (!area || questions.length === 0) {
    return { state, mode: 'questions_exhausted', cue: null, area, questions: [] };
  }
  return { state, mode: 'ask', cue: null, area, questions };
}
...
export function planBlock(plan) {
  ...
  if (plan.mode === 'wrap_up') {
    lines.push(`- 이 사진에 대한 질문 ${QUESTION_LIMIT}번을 모두 썼습니다. 질문하지 마십시오.`);
    lines.push('- 들려주신 이야기를 한 문장으로 짧게 정리하고,');
    lines.push('  잠시 쉬기 · 다른 사진 보기 · 오늘 마치기를 고르실 수 있다고 안내하십시오.');
    return lines.join('\n');
  }
  ...
  if (plan.mode === 'invite') {
    lines.push('- 두 차례 잇달아 질문했습니다. 이번에는 질문하지 마십시오.');
    lines.push('- 방금 하신 말씀을 한 문장으로 되받고, 무엇을 더 들려주시면 되는지 구체적으로 청하십시오.');
    lines.push('  ("그날 기억나는 장면부터 천천히 더 들려주세요." 처럼 — "더 말씀해 주세요" 처럼 막연하게 하지 않습니다)');
    return lines.join('\n');
  }
...
export function buildRecallPrompt({
  character = null, memory = null, analysis = null, memoryContext = '',
  history = [], lastUserText = '', cannotRecall = false, plan = null,
} = {}) {
  const made = plan || recallPlan({ history, lastUserText, analysis, memory, cannotRecall });
...
export function tidyRecallReply(reply, {
  history = [], userSaid = '', turnCue = null, plain = '그러셨군요.', mayAsk = true,
} = {}) {
