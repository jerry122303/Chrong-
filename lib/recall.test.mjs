...
import { limitToTwo, recallPlan, planBlock } from './recall-chat.js';
...
t('숫자는 프롬프트와 코드가 같다', [QUESTION_LIMIT, CONSECUTIVE_LIMIT], [5, 2]);
t('마무리 인사는 정해진 문장',
  STOP_RESPONSE, '네, 오늘 이야기는 여기서 마칠게요. 함께 이야기해 주셔서 감사합니다.');
...
t('첫 고정 질문도 한 번으로 센다', [first.questionCount, first.remaining], [1, 4]);
...
t('두 차례 잇달아 물었으면 이번엔 묻지 않는다',
  [twice.questionCount, twice.consecutive, twice.mayAsk], [2, 2, false]);
...
t('다섯 번을 다 쓰면 더 묻지 않는다', [full.questionCount, full.remaining, full.mayAsk], [5, 0, false]);
...
t('남은 횟수를 알려 준다', runtime.includes('남은 질문 횟수: 4번'), true);
t('첫 질문은 화면이 이미 드렸다고 알려 준다', runtime.includes('화면이 이미 드렸습니다'), true);
t('질문을 다 쓰면 정리하고 선택지를 안내하라고 한다',
  recallRuntime(full).includes('잠시 쉬기 · 다른 사진 보기 · 오늘 마치기'), true);
t('두 번 이어 물었으면 이야기 초대로 쓰라고 한다',
  recallRuntime(twice).includes("'반영 + 구체적인 이야기 초대'"), true);

console.log('\n[규칙을 어긴 답은 한 번 고쳐 받는다 (app.py _hard_violation)]');
t('물음표가 둘이면 고쳐 받는다',
  Boolean(hardViolation('누구와 가셨어요? 그곳은 어디였나요?', first)), true);
t('질문을 다 썼는데 또 물으면 고쳐 받는다',
  Boolean(hardViolation('그날 날씨는 어땠나요?', full)), true);
t('두 번 이어 물은 뒤 또 물으면 고쳐 받는다',
  Boolean(hardViolation('그때 기분은 어떠셨어요?', twice)), true);
t('JSON 을 내놓아도 고쳐 받는다', Boolean(hardViolation('{"next_action":"ask"}', first)), true);
t('규칙을 지킨 답은 그대로', hardViolation('바다에서 수영하셨군요. 그날 기억나는 장면을 들려주세요.', twice), null);
...
console.log('\n[기억이 안 나실 때 — 단서 사다리 (사람 → 장소 → 사물 → 선택형)]');
const ladder = cueLadder(beach);
t('사다리 차례', ladder.map((c) => c.step), ['person', 'place', 'object', 'object', 'choice']);
t('사람 단서는 몇 분인지만 말한다', ladder[0].line.includes('세 분'), true);
t('장소를 단정하지 않는다', ladder[1].line.includes('바닷가처럼 보이는'), true);
t('사진을 살펴보지 못했으면 단서가 없다', cueLadder(null), []);
const gaveOne = [OPEN, user('기억이 안 나'), bot(ladder[0].line), user('모르겠어')];
t('이미 드린 단서는 건너뛴다', nextCue(beach, gaveOne).step, 'place');
...
t('두 번 이어 물었으면 이야기 초대',
  recallPlan({ history: [OPEN, user('응'), bot('누구와 함께 계셨어요?'), user('딸')], analysis: beach }).mode,
  'invite');
t('다섯 번을 다 쓰면 정리하고 고르시게',
  recallPlan({ history: fiveAsked, analysis: beach }).mode, 'wrap_up');
t('할 일을 프롬프트 한 토막으로',
