...
t('JPEG 이 아니면 빈 값', readExif(Uint8Array.from([0x89, 0x50, 0x4E, 0x47, 1, 2, 3, 4])), { takenAt: null, dateSource: null, dateConfidence: null, gps: null });
t('잘린 파일도 던지지 않는다', readExif(exifJpeg().slice(0, 30)), { takenAt: null, dateSource: null, dateConfidence: null, gps: null });
t('쓰레기 바이트도 던지지 않는다', readExif(Uint8Array.from({ length: 300 }, (_, i) => (i * 37) & 0xFF)), { takenAt: null, dateSource: null, dateConfidence: null, gps: null });
t('정보가 없는 JPEG', readExif(Uint8Array.from([0xFF, 0xD8, 0xFF, 0xDA, 0, 2, 0xFF, 0xD9])), { takenAt: null, dateSource: null, dateConfidence: null, gps: null });
