...
import { cueSentence, factSource } from './reply-rules.js';
import { trustedDate } from './records.js';
...
/**
 * 사진 파일에 남은 찍은 날로 만드는 첫 단서.
 *
 * 시스템이 이미 알고 있는 사실이라 지어낸 말이 아니고, 한 해 · 한 달만 짚어 드리므로
 * 맞히게 하는 물음도 아니다. 그래서 사다리의 맨 앞에 둔다.
 * 다만 인화된 옛 사진을 휴대폰으로 다시 찍은 것으로 보이면 파일 날짜는 '오늘'이라
 * 쓰지 않는다 (사진 추천 점수에서도 같은 판단을 한다).
 */
function dateCue(memory, analysis) {
  const reprint = analysis && analysis.is_reproduction === true;
  /* 파일을 고친 때(LOW)는 찍은 때가 아닐 수 있어 단서로 쓰지 않는다 (lib/records.js trustedDate).
     카카오톡으로 받은 사진은 이 값이 '오늘' 인 경우가 많다 */
  const at = trustedDate(memory?.meta) ? String(memory.meta.taken_at) : '';
  /* 첫 단서에는 안심하는 말을 함께 둔다. 기억이 안 나신다고 하신 바로 뒤이기 때문이다 */
  const ease = '괜찮아요, 바로 떠오르지 않아도 돼요.';
  if (!reprint && /^\d{4}-\d{2}/.test(at)) {
    const [y, m] = at.split('-');
    return `${ease} 이 사진은 ${Number(y)}년 ${Number(m)}월에 찍은 사진인데, 그때 있었던 일이 떠오르시나요?`;
  }
  const year = Number(memory?.taken_year);
  if (Number.isInteger(year) && year >= 1900) {
    return `${ease} 이 사진은 ${year}년에 찍은 사진인데, 그때 있었던 일이 떠오르시나요?`;
  }
  return '';
}

/**
 * 단서 사다리 (찍은 날 → 사람 → 장소 → 사물 · 행동 → 쉬운 선택형).
 *
 * facts 는 확인된 사실(보호자나 어르신이 적어 두신 것)이다. 있으면 그 칸에서는
 * 사진을 보고 짐작한 말 대신 적어 둔 내용을 쓰고, 누가 적었는지 밝힌다.
 * 단서가 나가는 곳은 이 사다리 한 곳뿐이어야 한다 — 두 곳에서 나가면
 * 실제 대화에서 순서가 뒤엉켰다.
 */
export function cueLadder(analysis, memory = null, facts = null) {
  const steps = [];

  const dated = dateCue(memory, analysis);
  if (dated) steps.push({ step: 'date', line: dated });

  const known = (v) => (Array.isArray(v) ? v : [v])
    .map((x) => String(x ?? '').trim()).filter(Boolean);
  const toldPeople = known(facts?.people);
  const toldPlace = known(facts?.place)[0] || '';

  const done = analysis && analysis.status === 'DONE';
  const people = done ? Math.max(0, Math.round(Number(analysis.people_count) || 0)) : 0;
  const place = done ? String(analysis.place_label || '').trim() : '';
  const things = done && Array.isArray(analysis.objects)
    ? analysis.objects.map((o) => String(o).trim()).filter(Boolean) : [];
  const doing = done ? String(analysis.activity_label || '').trim() : '';

  if (toldPeople.length) {
    steps.push({
      step: 'person',
      line: `${cueSentence({ field: 'people', value: toldPeople.join(', '), source: factSource(memory, 'people') })} 이분들과 함께 계셨던 기억이 나세요?`,
    });
  } else if (people >= 2) {
    steps.push({
      step: 'person',
  ...
  /* 사진에 남은 좌표를 사람이 아는 이름으로 바꾼 것 (lib/geocode.js).
     이것은 파일에 실제로 적혀 있던 사실이라 그대로 말해도 된다 */
  const mapped = String(memory?.meta?.location_name || '').trim();

  if (toldPlace) {
    steps.push({
      step: 'place',
      line: `${cueSentence({ field: 'place', value: toldPlace, source: factSource(memory, 'place') })} 이곳에 갔던 기억이 나세요?`,
    });
  } else if (mapped) {
    steps.push({
      step: 'place',
      line: `이 사진은 ${mapped}에서 찍은 사진이에요. 이곳에 갔던 기억이 나세요?`,
    });
  } else if (place) {
...
export function nextCue(analysis, history = [], memory = null, facts = null) {
  const said = botSaid(history).map((m) => m.content).join('\n');
  const bare = (t) => String(t).replace(/[\s.?!,~…'"“”？]/g, '');
  const before = bare(said);
  /* 이미 건넨 단서는 건너뛴다. 적어 둔 내용은 낱말만 견주어도 알아본다 */
  const given = (c) => before.includes(bare(c.line))
    || (c.step === 'person' && (facts?.people || []).some((w) => before.includes(bare(w))))
    || (c.step === 'place' && facts?.place && before.includes(bare(facts.place)));
  return cueLadder(analysis, memory, facts).find((c) => !given(c)) || null;
}

/** 단서를 다 드렸는데도 기억이 안 나실 때 — 더 캐묻지 않는다 */
export const CUE_EXHAUSTED =
  '괜찮아요. 지금 바로 떠오르지 않아도 괜찮아요. 다른 사진을 함께 볼까요?';
...
export function summaryCard(history = []) {
  ...
  return {
    ...
    /* 남기면 무엇이 좋은지 함께 알려 드린다. 그냥 여쭈면 대개 그냥 넘기신다 (사용 피드백) */
    note: '저장하면 초롱이가 오늘의 추억을 기억하고, 다음 대화를 더 자연스럽게 이어갈 수 있어요.',
  };
}
