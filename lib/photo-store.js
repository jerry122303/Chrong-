...
export class PhotoStore {
  ...
  /**
   * 모델에게 보여 주려고 사진을 data URL 로 읽는다.
   *
   * 너무 큰 사진은 보내지 않는다. 화면에서 이미 긴 변 1600 으로 줄여 올리므로
   * 보통은 몇백 킬로바이트다. 그보다 크면 요청이 느려지고 비용도 커지는데,
   * 사진 이야기를 여는 데 그만한 해상도가 필요하지도 않다.
   *
   * @returns {Promise<string|null>} 못 읽거나 너무 크면 null
   */
  async dataUrl(name, maxBytes = 4 * 1024 * 1024) {
    const found = await this.open(name);
    if (!found) return null;
    if (found.bytes > maxBytes) {
      console.warn('[photo] 사진이 커서 모델에 보여 주지 않습니다', name, found.bytes);
      return null;
    }
    const buf = await fs.readFile(found.path);
    return `data:${found.type};base64,${buf.toString('base64')}`;
  }
