...
import { openingLine, photoRemark, RECALL_QUESTION } from '../public/recall-opening.js';
...
const opening = bot('이 사진을 보면 어떤 기억이 떠오르세요?');
...
console.log('\n[첫 말 — 보이는 것을 짚고, 늘 같은 첫 물음으로]');
t('장소와 사물 둘', openingLine(beach), `바닷가에서 찍은 사진이네요. 파라솔이랑 수박도 보이네요. ${RECALL_QUESTION}`);
t('받침에 따라 이랑 · 랑 · 이 · 가',
  [photoRemark({ status: 'DONE', objects: ['밥상', '한복'] }), photoRemark({ status: 'DONE', objects: ['자전거', '모자'] })],
  ['사진에 밥상이랑 한복이 보이네요.', '사진에 자전거랑 모자가 보이네요.']);
t('장소만', photoRemark({ status: 'DONE', place_label: '한옥 마당', objects: [] }), '한옥 마당에서 찍은 사진이네요.');
t('보이는 것 없이 여러 사람만', photoRemark({ status: 'DONE', people_count: 4, objects: [] }), '여러 분이 함께 계신 사진이네요.');
t('살펴본 결과가 없거나 살펴보는 중이면 첫 물음만',
  [openingLine(undefined), openingLine({ status: 'PENDING' })], [RECALL_QUESTION, RECALL_QUESTION]);
