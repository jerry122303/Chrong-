...
/* 추천 점수(lib/photo-score.js)로 고른다. 여기 기억들은 사진 파일도 분석도 없어서
   ① 중요도와 ⑦ 최근 사용만 점수가 된다. 옛 즐겨찾기는 ★★ 로 본다. */
t('점수가 가장 높은 사진이 먼저 (좋아하시는 사진 ★★)',
  (await mem.selectMemory('user_01')).title, '좋아하시는 사진');
t('점수가 같으면 아직 대화 안 한 사진이 먼저',
  (await mem.selectMemory('user_01', { excludeIds: [fav.memory_id] })).title, '딸의 결혼식');

await mem.markUsed(wedding.memory_id);
t('점수가 같으면 오래 쓰지 않은 사진이 먼저',
  (await mem.selectMemory('user_01', { excludeIds: [fav.memory_id] })).title, '오래 전에 본 사진');
...
console.log('\n[MEMORY — 같은 말씀을 되풀이하셔도 이번 회기에서 다시 여쭤 수 있다]');
const again1 = await mem.addClaim(old.memory_id, {
  field: 'when_text', value: '스무 해쯤 전', source: 'USER', session_id: 'session_1',
});
const again2 = await mem.addClaim(old.memory_id, {
  field: 'when_text', value: '스무 해쯤 전', source: 'USER', session_id: 'session_2',
});
t('같은 값은 새로 쌓지 않음', again1.claim_id, again2.claim_id);
t('여쭤 회기는 최신 것으로 옮김', again2.session_id, 'session_2');
