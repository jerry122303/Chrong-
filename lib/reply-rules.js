...
/* 문서 5번이 정한 첫 고정 질문. 화면이 이 말로 회상을 연다 */
export const RECALL_OPENING = '이 사진을 보니 어떤 기억이 떠오르시나요?';
 ...
 * 같은 분을 다르게 부르시는 말
 *
 * "손자" 라고 하셨다가 "손주" 라고 하시면 지금은 서로 다른 내용으로 쌓여,
 * 남길지 여쭐 때 같은 항목이 두 줄로 나온다 (실제 화면에서 그랬다).
 * 부르는 말이 통하는 것끼리 묶어 한 줄로 본다.
 * ------------------------------------------------------------------ */
const SAME_PERSON = [
  ['손주', '손자', '손녀'],
  ['딸', '따님'],
...
/** 같은 분을 가리키는 말이면 대표 낱말로 바꾼다 (손자 · 손녀 → 손주) */
export function canonicalWord(word) {
  const w = String(word ?? '').trim();
  const group = SAME_PERSON.find((g) => g.includes(w));
  return group ? group[0] : w;
}

/**
 * 두 값이 같은 내용인가.
 * 띄어쓰기와 부르는 말의 차이, 적은 차례는 같은 것으로 본다
 * ("딸, 사위" 와 "사위, 따님" 은 같다).
 */
export function sameValue(a, b) {
  const norm = (v) => (Array.isArray(v) ? v : [v])
    .map((x) => canonicalWord(String(x ?? '').replace(/\s+/g, '')))
