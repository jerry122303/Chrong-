...
### 사람 캐릭터 디자인 (보내 준 캐릭터 그림 기반)

보내 준 캐릭터 그림(준호 · 서연)을 SVG 로 옮겼습니다 (`public/avatar.js` 2장).
- 그림 좌표(가로 800 · 눈 높이 410)로 그리고 `ART_MATRIX` 로 한 번에 줄여 아바타 틀에 얹습니다.
  눈 · 눈꺼풀 · 입 기준점(`ART_GEOM`)도 그림 좌표이고, 머리 · 몸 기준점만 틀 좌표입니다.
- 준호: 옆으로 넘긴 갈색 삐죽 머리 · 까만 동그라미 눈 · 짙은 갈색 맨투맨
- 서연: 가운데 가르마 긴 머리(검은 윤곽선 · 머리결 선) · 반짝이 있는 눈 · 연보라 후드 + 흰 깃
- 입: 그림 속 웃는 입이 기본이고, 말할 때는 아래턱(`beak-lower`)만 내려가며 그 틈(`mouth-open`)이 벌어집니다.
- 눈썹과 팔은 그림에 없어서 두지 않습니다. 엔진이 찾는 부위라 빈 자리(`brow-l/r`, `wing-l/r`)만 남겨 둡니다.
...
5. 사람 캐릭터는 보내 준 그림을 손으로 옮긴 것이라 머리 모양 · 얼굴 폭이 원본과 조금 다를 수 있습니다.
   고칠 때는 `public/avatar.js` 의 `JUNHO` · `SEOYEON` 경로 값을 바꾸고 `expressions.html` 로 표정마다 확인합니다.
